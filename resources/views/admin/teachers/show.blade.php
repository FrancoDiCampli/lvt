            ...
            <table class="text-gray-700">
                ...
                <tbody>
                    @foreach ($alumnos ?? [] as $alumno)
                        <tr class="text-center">
                        <td  class="w-3/12">{{$alumno['student']->name}}</td>
                        @if (count($alumno['delivery'])>0)
                            <td  class="w-3/12 ">{{$alumno['delivery'][0]->created_at->format('d-m-Y')}}</td>
                            <td  class="w-3/12">{{$alumno['delivery'][0]->state}}</td>
                            <td  class="md:w-3/12 w-9/12">
                                <div class="flex justify-center">
                                    <a href="{{route('teacher.descargarDelivery', $alumno['delivery'][0]->file_path)}}" class="mx-1 text-teal-800">
